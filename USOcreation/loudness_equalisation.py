...
filename_core = 'uso_300ms_30_room-10-35-3'
folder_core = DIR / 'data' / 'distance_simulation' / '300ms' / 'logscale' / filename_core
...
normalised_folder_path_all = DIR / 'data' / 'distance_simulation' / 'all_300ms_normalised'
...
def generate_normalised_files(folder_path, duration=3.0, LUFS=-25.0):
    file_paths = [f for f in abs_file_paths(folder_path)]
    out_folder_path = folder_path / 'normalised'
...
def play_normalised_seq(save=False):
    normalised_file_paths = [f for f in abs_file_paths(normalised_folder_path_all)]
    #file_paths_choice = random.choices(normalised_file_paths, k=30)
    ...
    for normalised_file_path in seq:
        ...
        if not save:
            ...
            sound.data = sound.data[:44100]
            sound.play()
            input("ja?")
    if save:
        noise = slab.Binaural.pinknoise(duration=out.n_samples, level=40, kind="diotic")
        out = out + noise
        out.write(normalised_folder_path_all / 'stitched.wav', normalise=True)
        print("finished")
